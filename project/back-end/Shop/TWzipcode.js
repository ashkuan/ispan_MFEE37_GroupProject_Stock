...
(function (root, factory) {
  ...
  function TWzipcode(container, options) {
    ...
    if (
      ...
      Object.keys(this.options.database).length !== 0
    ) {
  ...
  TWzipcode.prototype.createDropdownList = function (el, id, role, opt) {
      ...
      for (c in self.database) {
        if ("undefined" !== typeof self.database[c]) {
          if (-1 === hide.indexOf(c)) {
            ...
            html.push(
              ['<option value="', c, '" ', selected, ">", c, "</option>"].join(
                ""
              )
            );
          }
    ...
    function onCountyChange(evt) {
      ...
      if (
        zipcode &&
        opts.hasOwnProperty("onSelect") &&
        "function" === typeof opts.onSelect
      ) {
      ...
      if (district.length) {
        if (self.database.hasOwnProperty(value)) {
          for (cn in self.database[value]) {
            if (
              (Array.isArray(hide) && !hide.length) ||
              (hide.length && -1 === hide.indexOf(cn))
            ) {
              selected = cn === nv || cn === sv ? ' selected="selected"' : "";
              combine = self.getOpt("combine")
                ? self.database[value][cn] + " "
                : "";
              sub.push(
                [
                  '<option value="',
    ...
    function onDistrictChange(evt) {
      ...
      if (elCounty && elDistrict && elZipcode) {
        if (
          self.database.hasOwnProperty(elCounty.value) &&
  ...
  TWzipcode.prototype.createInput = function (el, id, role, opt) {
    ...
    on(dom, "keyup", function (evt) {
      ...
      if (
        code &&
        code.hasOwnProperty("county") &&
        code.hasOwnProperty("district")
      ) {
            ...
            if (
              ...
              postal =
                resp.results[0].address_components[
                  resp.results[0].address_components.length - 1
                ].long_name;
  ...
  TWzipcode.prototype.createElements = function () {
    ...
    Array.prototype.forEach.call(container, function (el) {
      ...
      if (
        opts.hasOwnProperty("created") &&
        "function" === typeof opts.created
      ) {
